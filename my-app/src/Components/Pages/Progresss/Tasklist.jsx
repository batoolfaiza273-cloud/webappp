...
const TaskList = () => {
  ...
  return (
    <div style={{ maxWidth: '700px', margin: '0 auto', padding: '20px' }}>
      ...
      <h3 style={{ marginTop: '30px', backgroundColor:" #EAEBD0",paddingBottom:"10px",fontWeight:"bold" }}>All Tasks</h3>
      {tasks.length === 0 ? (
          ...
          <div key={task._id} style={{ display: "flex", justifyContent: "space-between", marginBottom: "10px" }}>
            ...
            <div>
              {task.status !== "completed" && (
                <button className="button-28" onClick={() => completeTask(task._id)}>Complete</button>
              )}
              <button className="button-28" onClick={() => deleteTask(task._id)} style={{ marginLeft: "5px" }}>Delete</button>
            </div>
          </div>
        ))
      )}
